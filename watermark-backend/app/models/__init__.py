...
class EmbedTask(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=_uuid)
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id", ondelete="CASCADE"))
    source_image_id: Mapped[str] = mapped_column(String(36), ForeignKey("images.id", ondelete="RESTRICT"))
...
class VerifyLog(Base):
    ...
    embed_task_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("embed_tasks.id", ondelete="SET NULL"), nullable=True
    )
